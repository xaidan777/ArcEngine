...
const Lab = {
    ...
    init() {
        ...
        this.camera.home();
        window.addEventListener('constants-changed', (e) => this.onConstant((e.detail && e.detail.name) || ''));
        window.addEventListener('lang-changed', () => { this.renderHint(); this.updateInfo(); });
        requestAnimationFrame(t => this.tick(t));
    },

    bindUi() {
        for (const btn of document.querySelectorAll('#camera-modes button')) {
        ...
        document.getElementById('opt-toon').addEventListener('change', (e) => {
            Inspector.apply(Inspector.fieldByName.WORLD3D_TOON, e.target.checked ? 1 : 0);
        });
        this.syncToonToggle();
    ...
    syncToonToggle() {
        document.getElementById('opt-toon').checked = Number(window.WORLD3D_TOON) > 0;
    },
