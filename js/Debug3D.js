...
const Debug3D = {
    ...
    // copies: from how many meshes sharing one geometry a group is worth reporting. 200 is the
    // threshold from the upstream kit: below it a scene is still fine, above it the group is
    // worth one instanced draw call.
    ...
    // Group meshes that look like copies of one model, so the linter can say "these 400 meshes
    // are one model" instead of leaving the author to notice. `limit` is the count from which
    // the group is worth reporting. Sorted largest first: the biggest win is the first fix.
    //
    // This is the actionable half of the Instances3D pairing: the linter points at the meshes,
    // World3D.addInstances draws them in one call.
    ...
    async lint(view, opts) {
        ...
        const notReady = [];
        /** @type {{ key: string, name: string }[]} meshes that could be one instanced group */
        const separateCopies = [];
        for (const m of scene.meshes) {
            ...
            // A mesh that is NOT already instanced but shares geometry with others is a copy
            // candidate; collected here and reported as groups at the end.
            if (!mesh.hasThinInstances) {
                separateCopies.push({
                    key: mesh.getTotalVertices() + 'v/' + mesh.getTotalIndices() + 'i',
                    name: mesh.name,
                });
            }
        ...
        // Copies of one model drawn as separate meshes: the single biggest CPU win available, and
        // the one an author cannot see by looking at the scene. Reported as groups, largest first.
        for (const g of this.copyGroups(separateCopies, this.LIMITS.copies)) {
            add('warn', 'many-copies', g.name, g.count + ' separate meshes look like copies of one model (' + g.key + '): each is its own draw call in the main pass, the shadow map, the outline mask and the ink edges — 2 to 15 µs of CPU per frame. Draw them with World3D.addInstances(view, mesh, kind, items) — one draw call for all');
        }
